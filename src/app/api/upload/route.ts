...
const CF_IMAGES_HASH = process.env.CLOUDFLARE_IMAGES_HASH!; // delivery hash
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Require signed URLs for gallery images
    if (type === "gallery") {
    ...
    // Return image ID and delivery URLs
    return NextResponse.json({
      imageId,
      urls: {
        // Different variants via Cloudflare Images transforms
        thumbnail: `https://imagedelivery.net/${CF_IMAGES_HASH}/${imageId}/thumbnail`,
        card: `https://imagedelivery.net/${CF_IMAGES_HASH}/${imageId}/card`,
        hero: `https://imagedelivery.net/${CF_IMAGES_HASH}/${imageId}/hero`,
        original: `https://imagedelivery.net/${CF_IMAGES_HASH}/${imageId}/public`,
      },
    });
  } catch (e: any) {
...
// DELETE /api/upload?imageId=xxx — Delete image from Cloudflare
...
// Helper to build Cloudflare image URL (use in components)
export function cfImageUrl(imageId: string, variant: "thumbnail" | "card" | "hero" | "public" = "card") {
